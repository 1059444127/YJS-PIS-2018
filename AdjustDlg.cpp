...
void CAdjustDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_CM_BAYER, m_ctlBayer);
...
BEGIN_MESSAGE_MAP(CAdjustDlg, CDialog)
	...
	ON_CBN_SELCHANGE(IDC_CM_BAYER, OnSelchangeCmBayer)
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
{	
	...
	m_ctlBayer.SetCurSel(m_bayerdata-1);
...
BOOL CAdjustDlg::OnInitDialog() 
{
	...
	m_ctlBayer.AddString("GBRG");
	m_ctlBayer.AddString("GRBG");
	m_ctlBayer.AddString("BGGR");
	m_ctlBayer.AddString("RGGB");
...
void CAdjustDlg::ResetHVDevice()
{
	...
	pFrame->SetExposureTime(pFrame->nMaxWidMd0,m_lTintUpper,m_lTintLower);
...
void CAdjustDlg::OnOK() 
{
	...
	pFrame->m_Layout        = (HV_BAYER_LAYOUT)m_bayerdata;
...
void CAdjustDlg::OnSelchangeCmBayer() 
{
	// TODO: Add your control notification handler code here
	CMainFrame *pFrame = (CMainFrame *)AfxGetMainWnd();
	ASSERT(pFrame);
	
	m_bayerdata = m_ctlCMADC.GetCurSel() + 1;
}
